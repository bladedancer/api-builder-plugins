...
describe('api-builder-plugin-gm-resthook', () => {
	let flownodes;
	before(() => {
	...
	describe('#constructor', () => {
		it('[TEST-1] should define flownodes', () => {
			...
			expect(mocknode('gm-resthook')).to.exist;
		});
	...
	describe('#todo', () => {
		it('[TEST-3] should fail to with invalid argument', () => {
			// invoke gm-resthook#todo and check that the default callback is
			// called: cb('invalid argument')
			return mocknode(flownodes).node('gm-resthook').invoke('todo', { todo: undefined })
				.then((data) => {
		...
		it('[TEST-4] should succeed', () => {
			// invoke gm-resthook#todo and check that the 'next' output callback is
			// called: cb.next(null, 'todo')
			return mocknode(flownodes).node('gm-resthook').invoke('todo', { todo: 'stuff' })
				.then((data) => {
